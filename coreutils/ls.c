...
/* -Cadil1  Std options, bbx always supports */
/* -gnsxA   Std options, bbx always supports */
/* -Q       GNU option, bbx always supports */
/* -k       SELinux option, bbx always supports (ignores if !SELinux) */
/*          Std has -k which means "show sizes in kbytes" */
/* -LHRctur Std options, bbx optionally supports */
/* -Fp      Std options, bbx optionally supports */
/* -SXvhTw  GNU options, bbx optionally supports */
/* -T WIDTH Ignored (we don't use tabs on output) */
/* -KZ      SELinux mandated options, bbx optionally supports */

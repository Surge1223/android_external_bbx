		...
		IF_FEATURE_INDIVIDUAL(, char **argv))
{
	...
	/* Redundant for bbx (run_applet_and_exit covers that case)
	 * but needed for "individual applet" mode */
	if (argv[1]
	 && !argv[2]
	 && strcmp(argv[1], "--help") == 0
	 && strncmp(applet, "bbx", 7) != 0
	) {
 ...
 * for bbx binary, not "individual" binaries.
 * However, keeping it here and linking it into libbbx.so
 * (together with remaining tiny applets/applets.o)
 * makes it possible to avoid --whole-archive at link time.
 * This makes (shared bbx) + libbbx smaller.
...
static void parse_config_file(void)
{
	...
	static const char config_file[] ALIGN1 = "/etc/bbx.conf";
	...
	while (1) {
		...
		if (section == 1) {		/* SUID */
			 ...
			 * up when the bbx configuration is changed. */
		 ...
		 * are used in some future version of bbx. */
		if (!section) {
...
static void install_links(const char *bbx, int use_symbolic_links,
		char *custom_install_dir)
{
	/* directory table
	 * this should be consistent w/ the enum,
	 * bbx.h::bb_install_loc_t, or else... */
	...
	for (i = 0; i < ARRAY_SIZE(applet_main); i++) {
		fpc = concat_path_file(
				...
				APPLET_NAME(i));
		// debug: bb_error_msg("%slinking %s to bbx",
...
/* If we were called as "bbx..." */
static int bbx_main(char **argv)
{
	if (!argv[1]) {
 ...
 help:
		...
		full_write2_str(
			...
			"\n"
			"Usage: bbx [function [arguments]...]\n"
			"   or: bbx --list"IF_FEATURE_INSTALLER("[-full]")"\n"
			IF_FEATURE_INSTALLER(
			"   or: bbx --install [-s] [DIR]\n"
			)
			"   or: function [arguments]...\n"
			"\n"
			"\tBusyBox is a multi-call binary that combines many common Unix\n"
			"\tutilities into a single executable.  Most people will create a\n"
			"\tlink to bbx for each function they wish to use and BusyBox\n"
...
# endif
	...
	if (ENABLE_FEATURE_INSTALLER && strcmp(argv[1], "--install") == 0) {
		...
		bbx = xmalloc_readlink(bb_bbx_exec_path);
		if (!bbx) {
			/* bb_bbx_exec_path is usually "/proc/self/exe".
		...
		install_links(bbx, use_symbolic_links, argv[2]);
		return 0;
	}
	...
	if (strcmp(argv[1], "--help") == 0) {
		...
		/* "bbx <applet> arg1 arg2 ..." */
		argv++;
	}
	/* We support "bbx /a/path/to/applet args..." too. Allows for
	 * "#!/bin/bbx"-style wrappers */
...
#endif
{
	...
	if (argv[1] && strncmp(argv[0], "bbx", 7) == 0) {
		/* "bbx <applet> <params>" should still work as expected */
		argv++;
	}
...
#else
	lbb_prepare("bbx" IF_FEATURE_INDIVIDUAL(, argv));

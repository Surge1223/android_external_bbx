 ...
 *      Erik Andersen    <andersen@codepoet.org> (Majorly adjusted for bbx)
 *
...
static void ask_terminal(void)
{
	 ...
	 * Testcase: "sleep 1; bbx ash" + press and hold [Enter].
	 * Result:
	 * ~/srcdevel/bbox/fix/bbx.t4 #
	 * ~/srcdevel/bbox/fix/bbx.t4 #
	 * ^[[59;34~/srcdevel/bbox/fix/bbx.t4 #  <-- garbage
	 * ~/srcdevel/bbox/fix/bbx.t4 #
	 *
	 * Checking for input with poll only makes the race narrower,
	 * I still can trigger it. Strace:
	 *
	 * write(1, "~/srcdevel/bbox/fix/bbx.t4 # ", 33) = 33
...
#if ENABLE_FEATURE_EDITING_ASK_TERMINAL
	if (S.sent_ESC_br6n) {
		/* "sleep 1; bbx ash" + hold [Enter] to trigger.

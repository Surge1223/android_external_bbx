 ...
 http:
	...
	if (!sp) {
		...
		*sp = '\0';
		...
		// http://bbx.net?login=john@doe is a valid URL
		// memmove converts to:
		// http:/bbx.nett?login=john@doe...
...
static FILE* prepare_ftp_session(FILE **dfpp, struct host_info *target, len_and_sockaddr *lsa)
{
	...
	if (!target->user)
		target->user = xstrdup("anonymous:bbx@");

		...
		if (pfd[FROM_CGI].revents) {
			...
			if (out_cnt >= 0) {
				...
				if (out_cnt >= 8 && memcmp(rbuf, "Status: ", 8) == 0) {
					 ...
					 * echo -en "Location: http://www.bbx.net\r\n"
					 * echo -en "\r\n"
					 */
					count = out_cnt;
					out_cnt = -1; /* buffering off */
		...
		const char *content_type)
{
	...
	putenv((char*)"SERVER_SOFTWARE=bbx httpd/"BB_VER);
...
int httpd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/* Undo bbx.c: we want to speak English in http (dates etc) */
	setlocale(LC_TIME, "C");
#endif

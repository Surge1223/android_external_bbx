...
%package petitboot
Group: System Environment/Shells
Summary: Version of bbx configured for use with petitboot
...
bootloader used on PlayStation 3. The bbx package provides a binary
...
%build
# create static bbx - the executable is kept as bbx-static
...
cd ..
# we are back in bbx-NN.MM dir now
cp %{SOURCE1} .config
...
if test "$arch"; then \
    ...
    make V=1 \
        ...
        CFLAGS_bbx="-static -nostartfiles -LuClibc-0.9.30.1/installed/lib uClibc-0.9.30.1/installed/lib/crt1.o uClibc-0.9.30.1/installed/lib/crti.o uClibc-0.9.30.1/installed/lib/crtn.o"; \
else \
    cat .config && \
...
# create bbx optimized for petitboot
make clean
...
cp bbx bbx.petitboot

%install
rm -rf $RPM_BUILD_ROOT
mkdir -p $RPM_BUILD_ROOT/sbin
install -m 755 bbx.static $RPM_BUILD_ROOT/sbin/bbx
install -m 755 bbx.petitboot $RPM_BUILD_ROOT/sbin/bbx.petitboot

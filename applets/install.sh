...
if [ "$cleanup" = "1" ] && [ -e "$prefix/bin/bbx" ]; then
	inode=`ls -i "$prefix/bin/bbx" | awk '{print $1}'`
	sub_shell_it=`
		cd "$prefix"
...
rm -f "$prefix/bin/bbx" || exit 1
mkdir -p "$prefix/bin" || exit 1
install -m 755 bbx "$prefix/bin/bbx" || exit 1
...
for i in $h; do
	...
	if [ "$scriptwrapper" = "y" ]; then
		if [ "$swrapall" != "y" ] && [ "$i" = "/bin/sh" ]; then
			ln $linkopts bbx "$prefix/$i" || exit 1
		else
			rm -f "$prefix/$i"
			echo "#!/bin/bbx" >"$prefix/$i"
			chmod +x "$prefix/$i"
		fi
		echo "	$prefix/$i"
	else
		if [ "$2" = "--hardlinks" ]; then
			bb_path="$prefix/bin/bbx"
		else
			case "$appdir" in
			/)
				bb_path="bin/bbx"
